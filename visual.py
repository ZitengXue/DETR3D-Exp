...
class NuScenesExplorerMars(NuScenesExplorer):
    ...
    def render_sample_data(self,
                           ...
                           show_radar_raw_velo: bool = False,
                           i:int=0,
                           ) -> None:
        """
        ...
        if out_path is not None:
            root_dir = './output1'
            file_name = sd_record['filename'].split('/')[-1].split('.')[0]
            output1_path = os.path.join(root_dir, file_name)
            # out_path = "./output1/output1{i+1}"

            plt.savefig(output1_path, bbox_inches='tight', pad_inches=0, dpi=200)
...
def _test_pred(results_path):
    ...
    for sample_token in selected_keys:
        ...
        # selected_lidar_token = selected_sample['data']['LIDAR_BACK']
        selected_cam_token = selected_sample['data']['CAM_BACK']

        # nusc_exp.render_sample_pred(selected_lidar_token, results_dict[sample_token],i)
        # i+=1
        nusc_exp.render_sample_pred(selected_cam_token, results_dict[sample_token],i)
        i+=1
