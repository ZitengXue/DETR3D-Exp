...
class BiMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        bsz,num_cam, tgt_len, _ = vision.size()
        tmp=[]
        for i in range(bsz):
            tmp.append(self.v_proj(vision[i]) * self.scale)#6,30825,256  6,30825,1024
        query_states=torch.cat(tmp,dim=0).view(bsz,num_cam,-1,1024)#1,6,30825,1024
        key_states = self._shape(self.l_proj(lang), -1, bsz*num_cam)#6,26,256  6,26,1024  6,26,4,256,  6,4,26,256
        tmp=[]
        for i in range(bsz):
            tmp.append(self._shape(self.values_v_proj(vision[i]), -1, num_cam))
        value_v_states=torch.cat(tmp,dim=0).view(bsz,num_cam,self.num_heads,-1,256)#1,6,4,30825,256
        value_l_states = self._shape(self.values_l_proj(lang), -1, bsz*num_cam)
        proj_shape = (bsz * self.num_heads, -1, self.head_dim)
        proj_shape1 = (bsz * self.num_heads * num_cam, -1, self.head_dim)
        query_states = self._shape1(query_states, tgt_len,
                                   bsz).view(*proj_shape1)#1,6,30825,1024   24,30825,256
        ...
        if attn_weights.size() != (bsz * self.num_heads * num_cam, tgt_len, src_len):
            raise ValueError(
        ...
        if attention_mask_l is not None:
            ...
            attention_mask = attention_mask.expand(bsz*num_cam, 1, tgt_len, src_len)
            attention_mask = attention_mask.masked_fill(
                attention_mask == 0, -9e15)

            if attention_mask.size() != (bsz * num_cam, 1, tgt_len, src_len):
                raise ValueError('Attention mask should be of '
                                 f'size {(bsz * num_cam, 1, tgt_len, src_len)}')
            attn_weights = attn_weights.view(bsz * num_cam, self.num_heads, tgt_len,
                                             src_len) + attention_mask
            attn_weights = attn_weights.view(bsz * self.num_heads * num_cam, tgt_len,
                                             src_len)
        ...
        if attn_output_v.size() != (bsz * self.num_heads * num_cam, tgt_len,
                                    self.head_dim):
            raise ValueError(
        ...
        if attn_output_l.size() != (bsz * self.num_heads * num_cam, src_len,
                                    self.head_dim):
            raise ValueError(
        ...
        attn_output_v = attn_output_v.view(bsz, num_cam, self.num_heads, tgt_len,
                                           self.head_dim)
        attn_output_v = attn_output_v.transpose(2, 3)
        attn_output_v = attn_output_v.reshape(bsz, num_cam, tgt_len, self.embed_dim)

        attn_output_l = attn_output_l.view(bsz, num_cam, self.num_heads, src_len,
                                           self.head_dim)
        attn_output_l = attn_output_l.transpose(2, 3)
        attn_output_l = attn_output_l.reshape(bsz, num_cam, src_len, self.embed_dim)
