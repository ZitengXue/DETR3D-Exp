...
class DetrTransformerEncoderLayer2(BaseModule):
    """Implements encoder layer in DETR transformer.
    ...
    def forward(self, query: Tensor, query_pos: Tensor,
                key_padding_mask: Tensor, reference_points: Tensor, **kwargs) -> Tensor:
        """Forward function of an encoder layer.
                ...
                self.norms[1](
                    self.ffn(
                        self.norms[0](
                            ...
                            key_pos=query_pos[i],
                            key_padding_mask=key_padding_mask[i],
                            reference_points=reference_points[i],
                            **kwargs))))
                        )
        tmp=torch.cat(tmp,dim=0).view(bsz,num_cam,-1,256)
        return tmp
